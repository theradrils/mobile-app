...
async function modifyBuildGradleAsync(projectRoot, callback) {
  const buildGradlePath = _path().default.join(projectRoot, 'android', 'build.gradle');
...
async function modifyAppBuildGradleAsync(projectRoot, callback) {
  const buildGradlePath = _path().default.join(projectRoot, 'android', 'app', 'build.gradle');
...
async function modifyAndroidManifestAsync(projectRoot, callback) {
  const androidManifestPath = await _config().AndroidConfig.Manifest.getProjectAndroidManifestPathAsync(projectRoot);
...
async function modifyMainActivityJavaAsync(projectRoot, callback) {
  const mainActivityJavaPath = (0, _glob().sync)(_path().default.join(projectRoot, 'android/app/src/main/java/**/MainActivity.java'))[0];

  const mainActivityString = _fsExtra().default.readFileSync(mainActivityJavaPath).toString();
