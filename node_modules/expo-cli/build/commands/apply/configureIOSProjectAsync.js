...
async function modifyEntitlementsPlistAsync(projectRoot, callback) {
  ...
  const directory = _path().default.dirname(entitlementsPath);

  const filename = _path().default.basename(entitlementsPath, 'plist');
...
async function modifyExpoPlistAsync(projectRoot, callback) {
  ...
  try {
    await _xdl().IosPlist.modifyAsync(supportingDirectory, 'Expo', callback);
  } catch (error) {
    ...
    await _xdl().IosPlist.cleanBackupAsync(supportingDirectory, 'Expo', false);
  }
