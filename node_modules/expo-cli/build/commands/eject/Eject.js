...
async function ejectAsync(projectRoot, options) {
  ...
  const {
    hasNewProjectFiles,
    needsPodInstall
  } = await createNativeProjectsFromTemplateAsync(projectRoot, exp, pkg, tempDir); // Set this to true when we can detect that the user is running eject to sync new changes rather than ejecting to bare.
  ...
  await configureAndroidStepAsync(projectRoot); // Install CocoaPods
...
async function installNodeDependenciesAsync(projectRoot, packageManager, {
  ...
  try {
    ...
    installJsDepsStep.fail(_chalk().default.red(`Something when wrong installing JavaScript dependencies, check your ${packageManager} logfile or run ${_chalk().default.bold(`${packageManager} install`)} again manually.`)); // TODO: actually show the error message from the package manager! :O
...
function writeMetroConfig({
  ...
  try {
    ...
    _log().default.nested(`- You will need to add the ${_chalk().default.bold('hashAssetFiles')} plugin to your Metro configuration. ${_log().default.chalk.dim((0, _TerminalLink().learnMore)('https://github.com/expo/expo/blob/master/packages/expo-updates/README.md#metroconfigjs'))}`);
...
async function createNativeProjectsFromTemplateAsync(projectRoot, exp, pkg, tempDir) {
